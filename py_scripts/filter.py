...
def main(
    ...
    filter_(files, p_value_col, fold_change_col, output_dir, excel, split)
...
def filter_(files, p_value_col, fold_change_col, output_dir, excel, split):
    ...
    for f in track(files, description="Filtering..."):
        if excel:
            ...
            try:
                ...
                print(
                    f"Error loading as csv, please check {f} file type.\nException: {e}"
                )
                return
...
def combined(f, df, fold_change_col, p_value_col, output_dir):
    # Filter given fold changes and p-value
    try:
        cond = ((df[fold_change_col] < -1) | (df[fold_change_col] > 1)) & (
            df[p_value_col] < 0.05
        )
        combined_df = df[cond]
    except Exception as e:
